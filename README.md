...
```bash
./llm-server.sh              # 기본 (262K, 로깅 ON, Thinking 요청별 제어)
./llm-server.sh 1m           # 1M 컨텍스트
./llm-server.sh --no-log     # 로깅 없이 실행 (Thinking 제어 불가)
...
| `llm-server.sh` (API) | 요청에 `"no_think": false` → Thinking ON | O |
| `llm-server.sh` (API) | 요청에 `enable_thinking: false` | **X (무시됨)** |
...
API 서버(로깅 모드)에서는 **요청별로** Thinking을 제어할 수 있습니다.
기본값은 **Thinking OFF** — 구조화된 JSON 출력에 적합.

```bash
# 기본 (Thinking OFF) — JSON 출력이 필요한 앱에 적합
curl http://localhost:8080/v1/chat/completions \
  -H "Content-Type: application/json" \
  -d '{"messages":[{"role":"user","content":"안녕!"}],"max_tokens":200}'

# Thinking ON이 필요한 경우 (수학, 코딩 등)
curl http://localhost:8080/v1/chat/completions \
  -H "Content-Type: application/json" \
  -d '{"messages":[{"role":"user","content":"복잡한 수학 문제"}],"max_tokens":8000,"no_think":false}'
```

| 요청 | 동작 |
|------|------|
| `"no_think"` 생략 | Thinking OFF (기본) |
| `"no_think": true` | Thinking OFF |
| `"no_think": false` | Thinking ON |

프록시가 서버 응답에서 `<think>` 태그와 reasoning을 제거하고, content만 깔끔하게 반환합니다.

> 참고: `enable_thinking: false`는 mlx_lm.server에서 무시됩니다. `no_think`는 프록시 전용 파라미터입니다.
