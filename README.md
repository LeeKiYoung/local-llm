...
| `llm-server.sh --think` (API) | 서버 시작 시 Thinking ON | O |
| API 요청 파라미터 `enable_thinking` | **X (mlx_lm.server가 무시)** | X |
...
Thinking은 **서버 시작 시** 결정됩니다. 요청별 제어는 mlx_lm.server의 한계로 지원되지 않습니다.

```bash
# Thinking OFF (기본) — JSON 출력, MiroFish 등 앱 연동용
./llm-server.sh

# Thinking ON — 수학, 코딩 정확도가 중요할 때
./llm-server.sh --think
```

> 참고: API 요청의 `enable_thinking: false` 파라미터는 mlx_lm.server가 무시합니다. thinking 텍스트가 content에 섞여 나오는 문제가 있어, 서버 레벨에서 `--chat-template-args`로 제어하는 방식을 사용합니다.
