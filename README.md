...
| `llm-server.sh` (API) | 요청에 `"enable_thinking": true` → Thinking ON | O |
...
curl http://localhost:8080/v1/chat/completions \
  -H "Content-Type: application/json" \
  -d '{"messages":[{"role":"user","content":"복잡한 수학 문제"}],"max_tokens":8000,"enable_thinking":true}'
```

| 요청 | 동작 |
|------|------|
| `"enable_thinking"` 생략 | Thinking OFF (기본) |
...
> 참고: mlx_lm.server 자체는 `enable_thinking`을 무시하지만, 프록시가 이를 가로채서 응답을 가공합니다. Qwen3.5 표준 파라미터명을 그대로 사용합니다.
