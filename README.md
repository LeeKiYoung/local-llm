...
```bash
./llm-server.sh              # 기본 (262K, Thinking ON, 로깅 ON)
./llm-server.sh 1m           # 1M 컨텍스트
./llm-server.sh --no-think   # Thinking OFF (JSON 출력 등에 유용)
./llm-server.sh --no-log     # 로깅 없이 실행
./llm-server.sh 1m --no-think --no-log  # 조합 가능
...
| `llm-server.sh` (API) | `--no-think` 옵션으로 서버 시작 | O |
| `llm-server.sh` (API) | 요청에 `enable_thinking: false` | **X (무시됨)** |
| `llm-server.sh` (API) | 프롬프트에 `/no_think` | **X (무시됨)** |
...
**요청 파라미터 `enable_thinking: false`는 mlx_lm.server에서 무시됩니다.**
서버 시작 시 `--no-think` 옵션으로 끌 수 있습니다:

```bash
./llm-server.sh --no-think
```

이렇게 시작하면 모든 요청에 Thinking OFF가 적용됩니다.
구조화된 JSON 출력이 필요한 앱(MiroFish 등)에서는 `--no-think` 필수.
